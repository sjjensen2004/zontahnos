...
def create_icmp_probe(body: schema.Create, db: Session = Depends(get_db)):
    ...
    if service.get_probe_by_name(db, name=body.name):
    ...
    try:
        ...
        service.create_probe(db=db, probe_data=probe_data)
...
def delete_icmp_probe(body: schema.Delete, db: Session = Depends(get_db)):
    ...
    db_probe = service.get_probe_by_name(db, name=body.name)
    if not db_probe:
        raise HTTPException(status_code=404, detail="Probe not found")

    try:
        # Delete the probe in PostgreSQL
        service.delete_probe(db=db, probe_id=db_probe.id)
...
def update_icmp_record(body: schema.Update, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db_probe = service.get_probe_by_name(db, name=body.probe_name)
